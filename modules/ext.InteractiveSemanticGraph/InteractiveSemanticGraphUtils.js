isg.util = class {
    ...
    static getSmwQuery(root, properties) {
        ...
        var url = `/sfb1153/wiki/api.php?action=ask&query=[[${encodeURIComponent(root)}]]`;
        var propertiesVar = '';
        ...
        for (var i = 0; i < properties.length; i++) {
            propertiesVar += '|?' + encodeURIComponent(properties[i]); // + "=" + encodeURIComponent(properties[i]); //explicit label overwrites property display title. ToDo: extrakt label in result and get$
            //propertiesVar += '|?' + encodeURIComponent(properties[i] + ".Display title of") + "=" + encodeURIComponent(properties[i] + ".Display title of"); //explicit query for display title due to slo$
