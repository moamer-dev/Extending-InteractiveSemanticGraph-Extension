        ...
        $(".InteractiveSemanticGraph").each(function (index) {  //cus_properties array has been added to defaultOptions in order to declare the properties that admin wish to discard on the right click menu **Mohamed**
            if ($('.InteractiveSemanticGraph').length) { //check if div element(s) exist
                ...
                (async () => {
                    ...
                    async function getProperties(root, properties) {
                                    
                        return fetch(isg.util.getSmwQuery(root, properties))
                        .then(res => res.json())
                        .then(response => {
                          var printrequests_arr = response.query.printrequests;
                          var pro_obj = new Object();  //This is the global object of key bairs: key => Preferred label & value => Default Label 
                          var pro_labels = []; 
                          var pro_keys = [];
                  
                          for ( var x = 0; x < printrequests_arr.length; x++){
                          pro_labels.push(printrequests_arr[x].label);
                          pro_keys.push(printrequests_arr[x].key);
                          }
                  
                          for ( var x = 0; x < printrequests_arr.length; x++){
                              pro_obj[response.query.printrequests[x].label] = pro_keys[x];
                          }
                          glob_obj = pro_obj; 
                        })
                    }
                  
                    await getProperties(input.root, input.properties);
                ...
                //With a given nodeID the edges are set to the nodeID, else they are set to the root node.
                    ...
                    function fetchData(root, properties, nodeID, setGroup, setColor) {
                        ...
                        fetch(isg.util.getSmwQuery(root, properties))
                            ...
                            .then(data => {
                                ...
                                //Replacing the properties arry labels with Properties array prefered label using the global object **Mohamed**
                                
                                for (var x = 0; x < properties.length; x++){
                                    for (const [key, value] of Object.entries(glob_obj)) {
                                      
                                      if (properties[x] == value) {
                                        properties[x] = key;
                                        //new_arr.push(key); //active if you would like to group the preferred labels in a separate array
                                      }
                                    }
                                  } 
                                ...
                                for (var i = 0; i < properties.length; i++) {
                                    ...
                                    for (var j = 0; j < data.query.results[root].printouts[properties[i]].length; j++) {
                                        ...
                                        if (data.query.results[root].printouts[properties[i]][j].fulltext) {
                                            if (
                                              data.query.results[root].printouts[properties[i]][j].exists === "1"
                                            ) {
                                                if (data.query.results[root].printouts[properties[i]][j].fulltext.includes("# ")){ //Make Subobect Label Human Readable by removing UUIDV4 From its Label **Mohamed**
                                                    id = data.query.results[root].printouts[properties[i]][j]
                                                    .fulltext.slice(0,11);
                                                }
                                                else{
                                                    id =
                                                        data.query.results[root].printouts[properties[i]][j]
                                                        .fulltext;} //use pagename as id for pages 
                                        ...
                                        if (!isNonExistingPage && data.query.results[root].printouts[properties[i][j].fulltext]){// + ".Display title of"][j+labelOffset])  // Excluding Display title of as it is no longer exist in JSON response **Mohamed**
                                            if (data.query.results[root].printouts[properties[i][j]].fulltext.includes("# ")){
                                                label = data.query.results[root].printouts[properties[i][j]].fulltext.slice(0,11); //Clean Label by removing UUIDV4 if exist **Mohamed**
                                            }
                                        }// + ".Display title of"][j+labelOffset]; //explicit use property display title due to slow update of the displaytitle page field  // Also Excluded **Mohamed**
                                        else if (data.query.results[root].printouts[properties[i]][j].displaytitle)
                                         label = data.query.results[root].printouts[properties[i]][j].displaytitle; //use display title of pages
                                    ...
                                    /*
                                    for (var [key, value] of glob_entries) {
                                            //console.log(key + "=" + value);
                                        if (properties[i] == key){
                                            var edgeLabel = value;
                                        }
                                            
                                        else if (properties[i] !== key){ 
                                ...
                                for (var x = 0; x < properties.length; x++){
                                    for (const [key, value] of Object.entries(glob_obj)) {
                                      //console.log(key + "=" + value);
                                      if (network.groups._groupNames[x] == value) {
                                        network.groups._groupNames[x] = key;
                                        //new_arr.push(key);
                                      }
                                    }
                                  } */
                    ...
                    //Enabling Hide & Display for the prefered labels o the legend Box
                       
                    for (var x = 0; x < input.properties.length; x++){
                        for (const [key, value] of Object.entries(glob_obj)) {
                          //console.log(key + "=" + value);
                          if (input.properties[x] == value) {
                            input.properties[x] = key;
                            //new_arr.push(key);
                          }
                        }
                      } 
                    ...
                    // Replacing the default properties list labels with the preferred labels using the global object again as the following code is outside the function scope **Mohamed**
                    for (var x = 0; x < input.properties.length; x++){
                        for (const [key, value] of Object.entries(glob_obj)) {
                          //console.log(key + "=" + value);
                          if (input.properties[x] == value) {
                    ...
                    /*
                    var seemore_btn = document.createElement("span");
                    //seemore_btn.id = "showMoreBtn";
                    seemore_btn.innerHTML="readmore";
                    document.body.append(seemore_btn);
                      */
                        ...
                        if (timeDiff > 300) {
                            ...
                            if (network.getNodeAt({x: params.pointer.DOM.x, y: params.pointer.DOM.y})) { //node clicked
                                ...
                                .then(page_properties => {
                                        page_properties = page_properties.filter( function( el ) {return !input.cus_properties.includes( el );} ); //remove ignored properties
                                        ...
                                        for (var i = 0; i < page_properties.length; i++) {
                                            if (!page_properties[i].startsWith("_")) {
                                                ...
                                                var li = document.createElement("li");
                                                li.setAttribute('def_label', page_properties[i].replaceAll("_", " ")); //Set def_label attributes to the <li> in order to use it as a filter parameter then  **Mohamed**
                                              
                                              //li.dataset.action = page_properties[i].replaceAll("_"," ");
                                                
                                                for (const [key, value] of Object.entries(glob_obj)) {
                                                    if ( page_properties[i] == value) {
                                                        li.innerHTML = page_properties[i] = key; //Replace the property name on right click list **Mohamed**
                                                        li.dataset.action = page_properties[i] = key; //Replace the property name on legend box as it comes from data_action attribute  **Mohamed**
                                                    }
                                                    else{
